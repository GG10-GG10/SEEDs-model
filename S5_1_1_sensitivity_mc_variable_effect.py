...
# 功能说明 (S5_1)
# 用途: 执行 variable_effect 与 mc 两种敏感性实验。
# 输入: Scenario_config_new.xlsx + 基线参数/排放参数表。
# Sheet 读取映射:
# variable_effect 模式读取 `MC_effect_low_land_new`（可由 CONFIG["variable_effect"]["mc_sheet_prefer"] 覆盖）。
# mc 模式读取 `MC`（通过 load_mc_specs()，固定 sheet_name='MC'）。
# 输出: targets.csv, <group>/summary/samples.csv, MC/*.xlsx
...
CONFIG = {
    ...
    "mc": {
        ...
        # 简明中文：
        # uniform：独立均匀抽样（每个维度都在区间内等概率）
        # lhs：拉丁超立方，样本少时覆盖更均匀
        # lhs_antithetic：LHS + 对偶镜像，固定样本数下均值/分位数更稳
        # range：分层固定网格，含端点，适合小样本“扫范围”
        # halton：低差异序列，覆盖均匀性较好
        # mixed：lhs + uniform 混合，兼顾覆盖与随机性
        "sampling": {
            "method": "lhs_antithetic",
            # MC_effect sampling scope controls how the normalized random U is
            # shared before mapping through each row's Min_bound/Max_bound:
        ...
        # 营养(能量口径)软约束（MC only）: 允许小幅需求缺口/过剩，避免硬不可行
        # 若启用，max_slack_rate 限制总短缺/过剩占总需求(能量口径)的比例
        "nutrition_soft_constraints": {
            "enabled": False,
        ...
        # 土地软约束（MC only）：允许土地上限小幅超额（ha），并在目标中惩罚
        # enabled: False
        # max_over_cap_rate: 0.02 -> 允许最多 2% 超额
        # slack_penalty_per_ha: 1e10 -> 超额每 ha 的惩罚系数（需足够大）
        "land_soft_constraints": {
            "enabled": False,
            "max_over_cap_rate": 0.02,
            "slack_penalty_per_ha": 1e10,
        ...
        # Non-EF sampling u mode (applies to ALL bounds, not just Y2020_XX):
        # 控制非EF变量的抽样比例 u 如何在国家/商品间共享
        # "shared": 同一个 u 作用到所有国家/商品（更一致/易解释）
        # "per_country": 每个国家独立抽样
        # "per_commodity": 不同商品独立抽样（所有国家共享该商品的 u）
        # "per_country_commodity": 国家+商品都独立抽样
        "mc_non_ef_mode": "shared",
        # EF sampling u mode (applies to ALL bounds):
        # 控制 emission_factor 的 u 如何在国家/商品间共享（Process 再由 ef_process_mode 控制）
        # "shared": 同一个 u 作用到所有国家/商品
        # "per_country": 每个国家独立抽样
        # "per_commodity": 不同商品独立抽样（所有国家共享该商品的 u）
        # "per_country_commodity": 国家+商品都独立
        "mc_ef_mode": "shared",
        # EF u grouping by process:
        # 是否按 Process 进一步拆分 u
        # "all": 忽略 Process（默认）
        # "by_process": 按实际 Process 独立抽样
        "ef_process_mode": "all",
    },
...
def _configure_cfg(mc_cfg: Dict[str, object]) -> None:
    ...
    # non-EF Y2020_XX 处理方式：shared / per_country
