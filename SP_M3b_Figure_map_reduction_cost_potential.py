...
process_colors = {
    'Reduce Ruminate': '#fb9a99',
    'Improve yield rate': '#377eb8',
    'Manure management': '#a65628',
    'Improve feed efficiency': '#ff7f00',
    'Enteric fermentation management': "#a6cee3",
    'Improve fertilizer efficiency': '#e41a1c',
    'Rice cultivation': "#ae7bc8",
    'Crop residue management': '#b2df8a',
    'Reduce waste': '#33a02c',
}
process_aliases = {
    ...
    'Improve manure management': 'Manure managementt',
}
NO_FILL_NAMES = {
    "GREENLAND",
    "ANTARCTICA",
...
def _style_axes(ax: plt.Axes, bounds: List[float]) -> None:
    ...
    ax.set_xticklabels([_format_lon(v) for v in lon_ticks], fontsize=7)
    ax.set_yticklabels([_format_lat(v) for v in lat_ticks], fontsize=7)
...
def main() -> None:
    ...
    for col in MAP_COLS:
        if col not in gdf_plot.columns:
            continue
        out_path = FIG_DIR / f"Figure3_{col}_map.png"
