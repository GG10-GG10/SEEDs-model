...
def load_scenario_config(xlsx_path: str, universe: Universe) -> List[ScenarioEffect]:
    ...
    # 兼容 Scenario Element / Element unit 列名
...
def apply_scenario_to_data(effects: List[ScenarioEffect], scenario_id: str, universe: Universe, nodes: List[Node],
    ...
    keys:
      ...
      - ruminant_intake_cap[(country,commodity,year)] = cap_in_t  （按 2020 年基准 commodity 需求 × (1 + rate_t)）
    ...
    out = {
        ...
        'thermal_adaptation_multiplier_by': {},  # residual stress, (country, commodity, year)
        'thermal_adaptation_levers': [],          # auditable lever rows
    }
    # 预处理：反刍动物基准需求（2020）按 (country, commodity) 分开统计
    ...
    for eff in [e for e in effects if e.scenario_id==scenario_id]:
        ...
        # yield??
            ...
            'housing_microclimate',
            'cooling_heating',
            'breed_resilience',
            'diet_resilience',
            'thermal_stress_adaptation',
            'thermal_stress_multiplier',
        ):
            for y, val in timeline.items():
                if y <= base_year:
                    continue
                if eff.kind == 'thermal_stress_multiplier' or eff.unit == 'multiplier':
                    residual = float(val)
                else:
                    # rate/absolute values are adaptation efficiencies.
                    residual = 1.0 - float(val)
                residual = max(0.0, min(1.0, residual))
                for i in eff.countries:
                    for j in eff.commodities:
                        key = (i, j, y)
                        prior = float(out['thermal_adaptation_multiplier_by'].get(key, 1.0))
                        out['thermal_adaptation_multiplier_by'][key] = prior * residual
                        out['thermal_adaptation_levers'].append(
                            {
                                'country': i,
                                'commodity': j,
                                'year': y,
...
def _mc_element_to_kind(elem: str) -> Optional[str]:
    ...
    if not elem_l:
        return None
    if 'microclimate' in elem_l or ('housing' in elem_l and ('cool' in elem_l or 'heat' in elem_l)):
        return 'housing_microclimate'
    if 'cooling' in elem_l or 'heating' in elem_l:
        return 'cooling_heating'
    if 'breed' in elem_l and ('resilien' in elem_l or 'thermal' in elem_l):
        return 'breed_resilience'
    if 'diet' in elem_l and ('resilien' in elem_l or 'thermal' in elem_l):
        return 'diet_resilience'
    if 'thermal' in elem_l and ('multiplier' in elem_l or 'factor' in elem_l):
        return 'thermal_stress_multiplier'
    if 'thermal' in elem_l and ('stress' in elem_l or 'adapt' in elem_l):
        return 'thermal_stress_adaptation'
