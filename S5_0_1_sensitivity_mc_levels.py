...
# 功能说明（S5_0）
# 用途：基于 MC 抽样结果评估“不同变量对目标排放水平的相对重要性”。
# 输入：Scenario_config_new.xlsx 的 MC 配置 + 模型基线数据 + 管线输出排放结果。
# 流程：
# 1) 按 MC 配置抽样并生成 scenario_effects；
# 2) 逐样本调用 run_one_pipeline 计算 2080 年全球排放；
# 3) 在给定目标排放附近按相关性/加权方式计算变量重要性；
# 4) 汇总输出细项和分组重要性结果。
# 主要输出：
# summary/samples.csv
