...
def solve_with_luc_iteration(
    ...
    feed_crop_link_coeff_by_livestock: Optional[
    ...
    for iteration in range(1, max_iterations + 1):
        result = solve_linear_regional(
            ...
            feed_crop_link_coeff_by_livestock=feed_crop_link_coeff_by_livestock,
...
class LinearModelCache:
    ...
    net_import: Dict[Tuple[str, str, int], gp.Var]  # 区域净进口 {(region, commodity, year): var}
    ...
    abatement_cost_vars: Dict[Tuple[str, str, int, str, int], gp.Var]  # MACC 成本分段变量
    abatement_cost_caps: Dict[Tuple[str, str, int, str, int], gp.Constr]  # 成本分段上限约束
    abatement_req_vars: Dict[Tuple[str, str, int, str], gp.Var]  # 基准减排量(相对BASE)
    abatement_req_constr: Dict[Tuple[str, str, int, str], gp.Constr]  # 基准减排量约束
    abatement_costs: Dict[Tuple[str, str, int, str, int], float]      # 减排边际成本
    proc_cap_basecoeff: Dict[Tuple[str, str, int, str, int], Optional[float]]   # MACC 基准系数(常量上限则为None)
...
# 是否启用区域聚合（True=使用 Region_market_agg，False=直接按国家处理）
...
def load_historical_max_production(
    ...
    返回:
        Dict[(region_or_country, commodity), max_production_t] - 区域或国家级历史最大产量
...
def load_grassland_coefficients(
    ...
    Returns:
        {(region_or_country, livestock_commodity): ha_per_ton} 字典
    """
    logger = logging.getLogger(__name__)
    ...
    try:
        ...
        # 构建返回字典 {(region/country, species): ha_per_head}
        result = {}
...
def aggregate_nodes_to_regions(
    ...
    for n in nodes:
        ...
        # ：直接检查meta中是否有grassland_coef，而不是用旧的LIVESTOCK_COMMODITIES判断
...
def build_linear_regional_model(
    ...
    cropland_nonforest_expand_ratio: float = 1.0,
    pasture_nonforest_expand_ratio: float = 1.0,
    base_cropland_by_region: Optional[Dict[str, float]] = None,  # LUH2基期耕地面积 {(region): ha}
    base_grassland_by_region: Optional[Dict[str, float]] = None,  # LUH2基期草地面积 {(region): ha}
    base_forest_by_region: Optional[Dict[str, float]] = None,  # LUH2/FAO基期森林面积 {(region): ha}
    background_forest_to_cropland_by_region_year: Optional[Dict[Tuple[str, int], float]] = None,
    background_forest_to_grassland_by_region_year: Optional[Dict[Tuple[str, int], float]] = None,
    background_cropland_to_forest_by_region_year: Optional[Dict[Tuple[str, int], float]] = None,
    background_grassland_to_forest_by_region_year: Optional[Dict[Tuple[str, int], float]] = None,
    land_demand_calibration_mode: str = 'none',
    yield_by_region_comm_year: Optional[Dict[Tuple[str, str, int], float]] = None,  # 目标年份单产 {(region/country, commodity, year): t/ha}
    yield_t_per_ha_default: float = 3.0,
    ...
    ruminant_commodities: Optional[List[str]] = None,
    # 市场失衡限制
    max_slack_rate: Optional[float] = 0.1,  # 市场年度短缺/过剩存量上限：占总需求(能量口径)的比例（例如0.1=10%，None=不限制）
    max_shortage_slack_rate: Any = "inherit",
    max_excess_slack_rate: Any = "inherit",
    slack_penalty: Optional[float] = 1e6,
    disable_production_cost_term: bool = True,  # True: 去掉生产成本项（默认）
    production_cost_weight: float = 1.0,  # 可选：生产成本项权重
    # 交叉弹性项裁剪
    cross_terms_top_n: Optional[int] = None,
    cross_terms_scale: Optional[float] = None,
    # 单位成本法参数
    ...
    参数:
        ...
        feed_reduction_by: 饲料强度变化率 {(region, commodity, year): rate in [-1, 1]}
    ...
    if len(tiny_pairs) > 0:
        ...
        # 创建要过滤的(region, commodity)集合。显式未来生物能源需求必须保留：
        # 这些节点可能由 S4 以 Q0=D0=0 的占位行补入，若按基期规模过滤，
        # 需求会从区域市场平衡约束中静默消失。
        tiny_set = {
    ...
    feed_coeff_livestock_map: Dict[Tuple[str, str, str, int], float] = {}
    for key, val in (feed_crop_link_coeff_by_livestock or {}).items():
        try:
            region_key, feed_key, livestock_key, year_key = key
            value = float(val)
            ...
            norm_key = (
                ...
                str(feed_key).strip(),
                str(livestock_key).strip(),
                int(year_key),
            )
            feed_coeff_livestock_map[norm_key] = value
        except Exception:
            continue
    ...
    if feed_link_mode == 'off':
        feed_coeff_map = {}
        feed_coeff_livestock_map = {}
        feed_override_map = {}
        feed_livestock_set = set()
    else:
        logger.info(
            "[LINEAR] feed link: mode=%s base=%d coeff=%d coeff_by_livestock=%d override=%d livestock=%d",
            feed_link_mode,
            len(feed_base_by_rc),
            len(feed_coeff_map),
            len(feed_coeff_livestock_map),
    ...
    def _lookup_feed_coef_by_livestock(
        region: Any,
        feed_commodity: Any,
        livestock_commodity: Any,
        year: Any,
    ) -> float:
        try:
            key = (
                str(region).strip(),
                str(feed_commodity).strip(),
                str(livestock_commodity).strip(),
                int(year),
            )
        except Exception:
            return 0.0
        return float(feed_coeff_livestock_map.get(key, 0.0) or 0.0)
    ...
    def _build_dynamic_feed_expr(region: Any, commodity: Any, year: Any) -> Optional[gp.LinExpr]:
        ...
        try:
            ...
            year_i = year
        feed_expr: Optional[gp.LinExpr] = None
        if feed_coeff_livestock_map:
            species_expr = gp.LinExpr(0.0)
            species_terms = 0
            for livestock_commodity in feed_livestock_set:
                coefficient = _lookup_feed_coef_by_livestock(
                    region, commodity, livestock_commodity, year_i
                )
                if coefficient == 0.0:
                    continue
                supply_var = Qs.get((region, livestock_commodity, year_i))
                if supply_var is None:
                    supply_var = Qs.get(
                        (str(region).strip(), str(livestock_commodity).strip(), year_i)
                    )
                if supply_var is None:
                    continue
                species_expr += coefficient * supply_var
                species_terms += 1
            if species_terms:
                feed_expr = species_expr
        if feed_expr is None:
            feed_coef = _lookup_feed_coef(region, commodity, year_i)
            if feed_coef == 0.0:
                return None
    ...
    for key in Qs.keys():
        ...
        # ：未来年份使用历史基期（hist_end_year）的 Q0 作为基准
        ...
        # 常数项先只扣自身价格基准偏移。交叉项可能因缺少变量或数值阈值被过滤，
        # 只有真正进入方程的交叉项才在下面扣除其基准偏移。
        sum_cross_eps = sum(float(v) for v in epsS_row.values())
        a_s = Q0 * (1.0 + yield_adj + temp_adj - eps_s)
        
        # 自身价格系数: b_s = Q0 * ε_s / P0 (absolute-price form)
        b_s_abs = Q0 * eps_s / P0
        b_s = b_s_abs
        if use_relative_price:
            price_ref_self = float(price_ref_by_comm.get(j, 1.0) or 1.0)
    ...
    for key in Qd.keys():
        ...
        # ：未来年份使用历史基期（hist_end_year）的 D0 和 P0 作为基准
        base_key = (r, j, hist_end_year)
        if base_key in idx:
        ...
        # Phase 2: 饲料强度变化 feed_reduction_by
        # 如果有 feed intensity scenario，对饲料用途的商品需求乘以 (1 + rate)
        # 这里简化处理：将 rate 直接应用到 D0_adjusted
        feed_mult = 1.0
        if feed_reduction_by:
            rate = float(feed_reduction_by.get(key, 0.0) or 0.0)
            rate = max(-1.0, min(1.0, rate))  # 限制在 [-1, 1]
            feed_mult = 1.0 + rate
            if feed_link_mode == 'off':
    ...
    # 修改：按(country, commodity, year)单独约束每个commodity
    # 因为regional aggregation，需要将同region的country caps聚合
    ...
    if ruminant_intake_cap and method not in {'nutrition', 'nutrition_band', 'nutrition_anchor'}:
        ...
        # Step 1: 构建country??region映射（从nodes）
        country_to_region = {}
        for node in nodes:
        ...
        # Step 2: 聚合caps到region level: (region, commodity, year) ?? sum of caps
    ...
    if cost_method_key in {'off', 'none', 'skip', 'disabled'}:
        ...
        for key in Qs.keys():
            ...
            for proc in sorted(process_names):
                ...
                # 当前过程排放。e0_by_proc uses ktCO2e/t for compatibility with
            ...
            if has_macc and sum_e0 > 0:
                ...
                for proc, e0p in e0_map.items():
                    ...
                    for seg_idx, (_, row) in enumerate(dfp.iterrows()):
                        ...
                        # 约束: a <= delta_frac * baseline_emissions
    ...
    # 约束含义：未来年土地实际占用不超过基期物理总面积；
    # crop/grass expansion 负责把“目标土地需求变化”映射成净转化量；
    # actual land stock 必须仅由基期 stock + 转化流量决定，不能再混入 demand-delta，
    # 否则 rolling 时会把同一变化重复计入。
    ...
    # forest_nonneg: cropland_actual + grassland_actual + forest_actual <= base_total(含 nonforest扩展上限)
    ...
    # 基础目标拆分：
    # 1) feasibility/slack stage：仅最小化供需/贸易软约束
    # 2) cost stage：在最小 slack 解集上最小化成本/排放
    # 兼容单阶段时，仍保留原有加权总目标
    ...
    # 3) explicitly enabled production cost, abatement cost, and land-carbon
    #    cost then rank suppliers.
    ...
    # 可选生产成本代理：Σ(P0 × Qs)。disable=True 在所有需求模式下都严格关闭；
    # nutrition 模式如需供应地排序，必须由显式目标项提供，不在此暗中重开。
...
def solve_linear_regional(
    ...
    if status == gp.GRB.INFEASIBLE:
        if not enable_infeasible_iis:
            ...
            try:
                ...
                # IIS 方向性检查：需求过大(净进口受限) vs 供给过大(净出口受限)
    ...
    if solver_result_is_extractable(
        ...
        # ：如果总短缺异常大，输出前10大短缺商品
        if enable_verbose_logging and total_shortage_slack > 1e6:  # 短缺 > 1 Mt
            # 计算短缺占总供给的比例（重量）
...
def apply_linear_sample_updates(
    ...
    *,
    ...
    # 线性供给方程：Qs = Q0 * (1 + η_y*(Ymult-1) + η_temp*(Tmult-1) - ε_s) + ε_s*Q0/P0*Pc
    # + Σ[ε_sj*Q0/P0j*(Pj-P0j)]
    # MC 更新产率：Ymult_new = Ymult0 * yield_mult
    # 新的常数项只扣实际进入方程的交叉项基准偏移。
    ...
    # 线性需求方程：Qd = D0 * pop_effect * inc_effect * (1 - ε_d) + ε_d*D0_adj/P0*Pc
    ...
    # ：这里假设 cache 中有 rumi_intake_constr 字段
