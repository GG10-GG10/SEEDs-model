...
class FeedDemandOutputs:
    ...
    crop_feed_by_species: pd.DataFrame
...
def build_feed_demand_from_stock(*,
                                 ...
                                 thermal_activity_multiplier: Optional[Dict[Tuple[str, str, int], float]] = None,
                                 thermal_dm_intake_multiplier: Optional[Dict[Tuple[str, str, int], float]] = None,
    ...
    empty = FeedDemandOutputs(
        ...
        grass_requirement=pd.DataFrame(columns=[
            'M49_Country_Code','country','iso3','year','grass_tdm_base',
            'grass_tdm','grass_area_need_ha'
        ]),
        species_dm_detail=pd.DataFrame(columns=[
            'country','iso3','m49_code','year','commodity','species',
            'stock_head_base','stock_head','activity_head_multiplier',
            'kg_dm_per_head_base','kg_dm_per_head','dm_intake_multiplier',
            'feed_efficiency_multiplier',
            'dm_total_kg_base','dm_total_kg','grass_dm_kg_base','grass_dm_kg',
            'crop_dm_kg_base','crop_dm_kg'
        ]),
        crop_feed_by_species=pd.DataFrame(columns=[
            'M49_Country_Code','country','iso3','year','livestock_commodity',
            'species','feed_commodity','feed_t_base','feed_t'
        ])
    )
    ...
    def _normalise_thermal_multiplier(
        values: Optional[Dict[Tuple[str, str, int], float]],
    ) -> Dict[Tuple[str, str, int], float]:
        out: Dict[Tuple[str, str, int], float] = {}
        for key, val in (values or {}).items():
            try:
                country, commodity, year = key
                country_key = _parse_m49(country) or str(country).strip()
                value = float(val)
                if not np.isfinite(value) or value < 0.0:
                    continue
                out[(country_key, str(commodity).strip(), int(year))] = value
            except Exception:
                continue
        return out

    activity_mult = _normalise_thermal_multiplier(thermal_activity_multiplier)
    dm_intake_mult = _normalise_thermal_multiplier(thermal_dm_intake_multiplier)
    ...
    # ：comm_to_species的key是Item_Emis，value是Item_Feed_Map（species）
    ...
    if stock.empty:
        ...
        return empty
    stock['stock_head_base'] = stock['stock_head']
    stock['kg_dm_per_head_base'] = stock['kg_dm_per_head']
    thermal_keys = list(zip(
        stock['m49_code'].astype(str),
        stock['commodity'].astype(str),
        stock['year'].astype(int),
    ))
    stock['activity_head_multiplier'] = np.array(
        [activity_mult.get(key, 1.0) for key in thermal_keys], dtype=float
    )
    stock['dm_intake_multiplier'] = np.array(
        [dm_intake_mult.get(key, 1.0) for key in thermal_keys], dtype=float
    )
    stock['stock_head'] = stock['stock_head_base'] * stock['activity_head_multiplier']
    stock['kg_dm_per_head'] = (
        stock['kg_dm_per_head_base'] * stock['dm_intake_multiplier']
    )
    # FeedEfficiency is a livestock dry-matter intensity multiplier. Apply it
    # exactly once before the grass/crop split, while retaining TS activity and
    # intake multipliers as independent physical effects.
    ...
    stock['dm_total_kg_base'] = (
        stock['stock_head_base'] * stock['kg_dm_per_head_base']
    )
    stock['dm_total_kg'] = (
        stock['stock_head']
        * stock['kg_dm_per_head']
    ...
    stock['crop_ratio'] = stock['crop_ratio'].clip(lower=0.0, upper=1.0)
    stock['grass_dm_kg_base'] = stock['dm_total_kg_base'] * stock['grass_ratio']
    stock['crop_dm_kg_base'] = stock['dm_total_kg_base'] * stock['crop_ratio']
    stock['grass_dm_kg'] = stock['dm_total_kg'] * stock['grass_ratio']
    stock['crop_dm_kg'] = stock['dm_total_kg'] * stock['crop_ratio']

    crop_dm_rows = stock[[
        'country','iso3','m49_code','year','commodity','species',
        'dm_total_kg_base','dm_total_kg','crop_ratio'
    ]].rename(columns={'commodity': 'livestock_commodity'}).merge(
        crop_share,
        how='left',
    ...
    crop_dm_rows['crop_dm_kg_base'] = (
        crop_dm_rows['dm_total_kg_base'] * crop_dm_rows['crop_ratio'] * crop_dm_rows['share']
    )
    ...
    if crop_dm_rows.empty:
        crop_feed_demand = pd.DataFrame(columns=['M49_Country_Code','country','iso3','year','commodity','feed_t'])
        crop_feed_by_species = empty.crop_feed_by_species.copy()
    else:
        ...
        crop_dm_rows['feed_commodity'] = crop_dm_rows['crop'].map((maps.production_by_item or {}))
        crop_dm_rows['feed_commodity'] = crop_dm_rows['feed_commodity'].fillna(crop_dm_rows['crop'])
        crop_dm_rows['grain_need_kg_base'] = (
            crop_dm_rows['crop_dm_kg_base'] / crop_dm_rows['dm_fraction']
        )
        crop_dm_rows['grain_need_kg'] = crop_dm_rows['crop_dm_kg'] / crop_dm_rows['dm_fraction']
        crop_dm_rows = crop_dm_rows[crop_dm_rows['feed_commodity'].isin(universe.commodities)]
        crop_dm_rows['feed_t_base'] = crop_dm_rows['grain_need_kg_base'] / 1000.0
        ...
        crop_feed_by_species = crop_dm_rows.groupby(
            [
                'M49_Country_Code','country','iso3','year','livestock_commodity',
                'species','feed_commodity'
            ],
            as_index=False
        )[['feed_t_base','feed_t']].sum()
        crop_feed_demand = crop_feed_by_species.rename(
            columns={'feed_commodity': 'commodity'}
        ).groupby(
            ['M49_Country_Code','country','iso3','year','commodity'],
            as_index=False
        )['feed_t'].sum()

    grass_req = stock.groupby(['country','iso3','m49_code','year'], as_index=False)[
        ['grass_dm_kg_base','grass_dm_kg']
    ].sum()
    ...
    grass_req['grass_tdm_base'] = grass_req['grass_dm_kg_base'] / 1000.0
    ...
    species_dm_detail = stock[['country','iso3','m49_code','year','commodity','species',
                               ...
                               'feed_efficiency_multiplier',
                               'dm_total_kg_base','dm_total_kg','grass_dm_kg_base','grass_dm_kg',
                               'crop_dm_kg_base','crop_dm_kg']].copy()
    ...
    return FeedDemandOutputs(
        ...
        crop_feed_by_species=crop_feed_by_species,
    )
