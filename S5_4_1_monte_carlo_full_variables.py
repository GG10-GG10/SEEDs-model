# -*- coding: utf-8 -*-
"""
S5.4 功能脚本：基于 MC_effect_low_land_new 的全变量 Monte Carlo 抽样，批量调用主模型并输出：
1) 样本状态表；
2) 成功样本的 fast emissions summary；
3) 成功样本的全球尺度分 process CO2eq；
4) 成功样本的各 Element 全球 calorie 加权均值。

说明：
- 抽样规则直接复用 S5.1 中对 Scenario_config_new.xlsx / MC_effect_low_land_new 的解析与归一化。
- 为避免破坏现有功能链，仍调用 S4_0_main.run_one_pipeline(...) 执行完整情景。
- Element 加权均值使用基期 2020 production_t * kcal_per_ton 作为国家-商品权重。
"""
...
# 5 批并行示例配置：
# 说明：
# 5 个脚本/作业必须保持除 batch_index 外完全一致的配置，尤其是：
# samples / seed / sampling / mc_sheet_prefer / override_cfg 都要相同。
# 这样每个批次都会从同一套完整抽样矩阵中领取自己的 sample_id 子集；
# 全部批次合并后，等价于一次完整的 Monte Carlo 抽样。
...
# "total_batches": 5,
# "batch_index": 1,
# "assignment": "round_robin", # 推荐：各批次负载更均匀
# "batches_subdir": "batches",
# }

# 第 2~5 批只改 batch_index：
