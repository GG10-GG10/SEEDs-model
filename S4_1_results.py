...
成本输出：
- cost_summary.csv：国家-商品-过程/策略成本明细
...
def summarize_emissions_from_detail(emission_detail_df: pd.DataFrame,
    ...
    # 1.4 已禁用：历史dairy/non-dairy拆分逻辑（新输入文件已完成拆分）
    ...
    if False and 'Item' in df.columns and 'year' in df.columns and 'Process' in df.columns and production_df is not None:
        ...
        # ：只包含dict_v3中定义的单一动物（Buffalo, Camels, Goats, Sheep）
        ...
        # ：只拆分历史年份（<=2020）的livestock过程的merged数据
        ...
        if len(merged_rows) > 0:
            ...
            if 'stock' in production_df.columns and 'Item_Emis' in production_df.columns:
                # ：不处理'Sheep and Goats'等不在dict_v3中的项
    ...
    # 1.4.1 只对 livestock merged 项做严格过滤
    # 这里不能再对所有Item做全局dict_v3过滤，否则会把LUC新下沉的commodity Item一并删掉，
    # 导致 By_Country_Process_Item 看不到 LUC 的分Item结果。
    ...
    # ：按用户要求，Long表不包含Country和iso3列（只保留M49_Country_Code）
    ...
    # 汇总层不能把过程总项和分Item明细项一起加总，否则会在 Sheet2/3 放大。
    # 当前已确认的重复源：
    # 1. GCE 的 All Crops / All Animals
    # 2. GSOIL 的 aggregate item "Drained organic soils"
    df_agg = df.copy()
    ...
    def add_ghg_co2eq_rows(pivot_df: pd.DataFrame) -> pd.DataFrame:
        """向量化追加GHG换算行（CH4_CO2eq/N2O_CO2eq/CO2_CO2eq/CO2eq）。"""
...
def summarize_emissions(fao_results: Dict[str, Any],
    ...
    # ：标记哪些行是"商品基础"（应跳过dict_v3 Item_Emis过滤）
    # 规则：
    ...
    column_mapping = {
        ...
        'n2o_kt': 'N2O_kt',
        'co2_kt': 'CO2_kt',
        'CH4_kt': 'CH4_kt',  # ：GLE返回的是大写的CH4_kt
        'N2O_kt': 'N2O_kt',  # ：支持大写的列名
        'CO2_kt': 'CO2_kt',  # ：支持大写的列名
        'emissions_kt': 'value'
    }
    ...
    # ：确保其他关键列也是标量（Country, M49_Country_Code, GHG等）
    ...
    # ：处理混合格式数据（wide: CH4_kt/N2O_kt/CO2_kt + long: GHG/value）
    ...
    if has_gas_cols and has_ghg_value:
        ...
        # 1. 识别已经是long格式的行
        # ：使用module列辅助识别Long格式，防止LUC未来年份（value=NaN）被误判为Wide格式
        if 'module' in combined_df.columns:
            # LUC模块强制视为Long格式
...
def generate_cost_summary(var: Dict, unit_cost_data: Dict, baseline_scenario_result: Dict,
    ...
    # 提取变量。strategy_abatement_blocks 使用三元键
    # (region, year, database_strategy)，并以 tCO2e 为统一单位。
