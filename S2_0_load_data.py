...
_THERMAL_TEMPLATE_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "thermal_stress_templates"
)
...
class DataPaths:
    ...
    # External preprocessing computes nonlinear thermal indices on the 5 km
    # meteorology grid and joins AGLW livestock weights before model ingestion.
    thermal_exposure_path: str = os.path.join(
        get_input_base(), "ThermalStress", "processed", "thermal_exposure_5km.parquet"
    )
    thermal_exposure_manifest_path: str = os.path.join(
        get_input_base(), "ThermalStress", "processed", "thermal_exposure_manifest.json"
    )
    thermal_hourly_climate_path: str = os.path.join(
        get_input_base(), "ThermalStress", "raw", "thermal_hourly_climate_5km.parquet"
    )
    thermal_livestock_weights_path: str = os.path.join(
        get_input_base(), "ThermalStress", "raw", "livestock_weights_5km.parquet"
    )
    thermal_response_registry_path: str = os.path.join(
        get_input_base(), "ThermalStress", "parameters", "thermal_response_registry.csv"
    )
    thermal_strain_registry_path: str = os.path.join(
        get_input_base(), "ThermalStress", "parameters", "thermal_strain_registry.csv"
    )
    thermal_activity_factor_path: str = os.path.join(
        get_input_base(), "ThermalStress", "parameters", "thermal_activity_factors.csv"
    )
    thermal_pollutant_factor_path: str = os.path.join(
        get_input_base(), "ThermalStress", "parameters", "thermal_pollutant_factors.csv"
    )
    thermal_tier2_parameter_path: str = os.path.join(
        get_input_base(), "ThermalStress", "parameters", "thermal_tier2_parameters.csv"
    )
    thermal_commodity_map_path: str = os.path.join(
        _THERMAL_TEMPLATE_DIR, "thermal_species_commodity_map.csv"
    )
    # inputs
    ...
    livestock_patterns_csv: str = os.path.join(get_input_base(), "Manure_Stock", "Environment_LivestockManure_with_ratio.csv")  # ：指向正确的livestock stock文件
...
def load_land_cover_from_excel(excel_path: str,
    ...
    source = 'FAO':
      ...
      - 只保留 Element=Area & Item in {cropland, grassland, forest}
...
def apply_temperature_multiplier_to_nodes(temp_xlsx: str,
    ...
    def _fail(message: str) -> Dict[str, Any]:
        ...
        return {"applied": 0, "status": "skipped", "reason": message}

    if not os.path.exists(temp_xlsx):
        return _fail(f"Temperature driver file not found: {temp_xlsx}")
    logger = logging.getLogger(__name__)
    try:
        df = _lc(_read_excel(temp_xlsx, sheet_name=0))
    except Exception as exc:
        return _fail(f"Failed to read temperature file {temp_xlsx}: {exc}")
    ...
    if not (c_area and c_year and c_tmul):
        ...
        if not all(c in df.columns for c in ['SCENARIO', 'REGION', 'VARIABLE']):
            return _fail(
                "Temperature driver has neither country/year/Tmult nor IAM schema"
            )
        year_cols = [c for c in df.columns
                     if isinstance(c, str) and c.startswith('Y') and c[1:].isdigit()]
        if not year_cols:
            return _fail("Temperature IAM driver has no YYYYY columns")
        ...
        if not (chosen_scenario and chosen_variable and chosen_region):
            return _fail(
                "Temperature IAM selector did not match exactly: "
                f"scenario={scenario!r}, variable={variable!r}, region={region!r}; "
                f"available scenarios={scenarios!r}, variables={variables!r}, "
                f"regions={regions!r}"
            )
        ...
        if df_use.empty:
            return _fail(
                "Temperature filter is empty: "
                f"scenario={chosen_scenario!r} variable={chosen_variable!r} "
                f"region={chosen_region!r}"
            )
        ...
        for n in nodes:
            ...
            if v is not None:
                n.Tmult = float(v)
                applied += 1
            elif int(n.year) > 2020:
                missing_node_keys.add((str(n.country), int(n.year)))
        if strict and missing_node_keys:
            raise ValueError(
                "Temperature IAM driver does not cover node country/year keys: "
                + repr(sorted(missing_node_keys)[:20])
            )
        if applied == 0:
            return _fail("Temperature IAM driver applied to zero model nodes")
        logger.info(
    ...
    if strict and missing_node_keys:
        raise ValueError(
            "Temperature driver does not cover node country/year keys: "
            + repr(sorted(missing_node_keys)[:20])
        )
    if applied == 0:
        return _fail("Temperature driver applied to zero model nodes")
...
def assign_grassland_coef_to_nodes(nodes: List[Node],
    ...
    2. 优先使用yield×屠宰率计算ton/head（与GLE路径一致），转换为ha/ton
    3. 如yield数据缺失，再回退到production/stock估计ton/head，并分配到meta['grassland_coef']
    ...
    Args:
        ...
        stock_df: 可选的存栏数据（用于production/stock回退）
        livestock_yield_df: 可选的livestock产率数据（t/head，用于yield×屠宰率）
        yield_multiplier: 可选情景产率倍率（(country, commodity, year) -> mult）
    """
    ...
    try:
        ...
        # ：直接使用commodity_to_species的键作为livestock判断依据
        ...
        for n in nodes:
            ...
            if arg_m49 and n.country == arg_m49 and n.year == 2020:
                ...
                # 转换为 ha/ton - 优先yield×屠宰率，缺失时回退产量/存栏比
...
def attach_emission_factors_from_fao_modules(nodes: List[Node], params_wide: Optional[pd.DataFrame],
    ...
    # 约束：不能覆盖节点上已经存在的过程排放强度；否则占位模块返回空值时会把有效 e0_by_proc 清空。
    for n in nodes:
    ...
    for row in df.itertuples(index=False, name=None):
        ...
        for col, col_idx in eff_cols_idx:
            ...
            if pd.isna(val):
                continue
            records_eff.append({
                'M49_Country_Code': m49_normalized,  # ：使用标准化的M49代码
                'country': country,
                'iso3': iso3,
                'commodity': commodity,
                'year': year,
