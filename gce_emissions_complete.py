# -*- coding: utf-8 -*-
"""
gce_emissions_complete.py - 浣滅墿鎺掓斁瀹屾暣璁＄畻妯″潡锛堜笌 FAOSTAT 瀵归綈锛?

妯″潡瀹氫綅
--------
鏈ā鍧楀疄鐜颁綔鐗儴闂紙GCE锛夌殑瀹屾暣鎺掓斁璁＄畻寮曟搸锛屼笌 livestock 妯″潡绫讳技锛屾彁渚?
鏃犵姸鎬併€佸彲缁勫悎鐨勮绠楀嚱鏁般€?

瑕嗙洊鐨勫洓涓富瑕佽繃绋嬶細
1. Crop residues (鐩存帴 N2O) - 绉哥杩樼敯閲婃斁 N2O
2. Burning crop residues (CH4/N2O) - 绉哥鐒氱儳閲婃斁 CH4 鍜?N2O
3. Rice cultivation (CH4) - 绋讳綔姘寸敯閲婃斁 CH4
4. Synthetic fertilizers (N2O) - 鍚堟垚鑲ユ柦鐢ㄩ噴鏀?N2O

鏁版嵁娴侊細
  鐢熶骇閲?(production_t) + 鍙傛暟 (residue N content, EF) 
  鈫?娈嬩綋N鍚噺 or DM鍚噺 
  鈫?涔樹互鎺掓斁鍥犲瓙 
  鈫?鎺掓斁閲?(N2O_kt, CH4_kt)

鍏抽敭鏁版嵁鏉ユ簮锛?
  - 鍘嗗彶浜ч噺: Production_Crops_Livestock_E_All_Data_NOFLAG.csv
  - 鍘嗗彶鎺掓斁: Emissions_crops_E_All_Data_NOFLAG.csv
  - 鍙傛暟琛? Code/src/GCE_parameters.xlsx (GCE_parameters sheet)
  - dict_v3: 鐢ㄤ簬 Item 鍚嶇О鏄犲皠涓?M49 country 杩囨护
"""
...
class CropEmissionsCalculator:
    ...
    鑱岃矗锛?
    1. 浠?GCE_parameters.xlsx 璇诲彇鍙傛暟
    2. 浠庡巻鍙叉帓鏀?CSV 璇诲彇鍘嗗彶鏁版嵁
    3. 璁＄畻鏈潵骞翠唤鐨勫悇杩囩鎺掓斁
    4. 鏀寔鎯呮櫙鍜?MC 妯℃嫙
    """
    ...
    def __init__(self, 
        ...
        """
        鍒濆鍖?Crop Emissions Calculator
        
        Args:
            gle_params_path: GCE_parameters.xlsx 璺緞
            dict_v3_path: dict_v3.xlsx 璺緞
            hist_emissions_crop_path: Emissions_crops_E_All_Data_NOFLAG.csv 璺緞
            fertilizer_efficiency_path: Fertilizer_efficiency.xlsx 璺緞锛堢敤浜嶴ynthetic fertilizers鍘嗗彶鍒嗛厤锛?
        """
        ...
        # 鍔犺浇鍙傛暟琛紙GCE_parameters锛?
        self._load_gce_parameters()
        
        # 鍔犺浇鍘嗗彶鎺掓斁鏁版嵁锛堢敤浜庡巻鍙插勾浠界洿鎺ヨ鍙栵級
        self._load_historical_emissions()
        
        # 鍔犺浇鑲ユ枡鏁堢巼鏁版嵁锛堢敤浜嶴ynthetic fertilizers鍘嗗彶鍒嗛厤锛?
    ...
    def _load_historical_emissions(self) -> None:
        """浠?Emissions_crops_E_All_Data_NOFLAG.csv 鍔犺浇鍘嗗彶鎺掓斁鏁版嵁"""
    ...
    def _load_fertilizer_efficiency(self) -> None:
        """浠?Fertilizer_efficiency.xlsx 鍔犺浇鑲ユ枡鏁堢巼鏁版嵁"""
    ...
    def _standardize_results(self, results: List[Dict]) -> pd.DataFrame:
        """
        灏嗚绠楃粨鏋滄爣鍑嗗寲涓虹粺涓€鐨勫垪缁撴瀯
        杩斿洖: M49_Country_Code, Item, year, process, CH4_kt, N2O_kt, CO2_kt
        """
        if not results:
            return pd.DataFrame()
    ...
    def get_parameter(self, 
        ...
        鏌ユ壘閫昏緫锛?
        1. M49 绮剧‘鍖归厤 + Item + Process + ParamName
        2. 鑻ユ棤锛屽皾璇?M49='000' (Global) 鍥為€€
        3. 鑻ョ洰鏍囧勾浠藉垪涓嶅瓨鍦ㄦ垨鍊间负NaN锛屽悜鍓嶆煡鎵炬渶杩戝彲鐢ㄥ勾浠界殑鍊?
        """
        # 瑙勮寖鍖?M49
        m49 = _norm_m49(m49_code)
        
        # 璇嗗埆鎵€鏈夊勾浠藉垪锛堟寜骞翠唤鎺掑簭锛?
        year_map = {}
        ...
        def _find_value_in_row(row: pd.Series, target_year: int) -> Optional[float]:
            """
            浠庤涓煡鎵惧弬鏁板€硷紝鏀寔骞翠唤澶栨帹鍜孨aN澶勭悊
            
            绛栫暐锛?
            1. 灏濊瘯鐩爣骞翠唤
            2. 濡傛灉涓嶅瓨鍦ㄦ垨涓篘aN锛屽悜鍓嶆煡鎵炬渶杩戝彲鐢ㄥ勾浠?
            """
            # 骞翠唤鍒楀悕锛氱洿鎺ョ敤瀛楃涓?2000", "2020"绛夛紙涓嶅甫Y鍓嶇紑锛?
            for col in year_map.get(target_year, []):
                if col in row.index and pd.notna(row[col]):
                    return float(row[col])
            available_years = [y for y in year_cols if y <= target_year]
            if not available_years:
                # 濡傛灉鐩爣骞翠唤鏃簬鎵€鏈夋暟鎹紝浣跨敤鏈€鏃勾浠?
                available_years = [min(year_cols)]
            
            # 浠庢渶杩戝勾浠藉悜鍓嶆煡鎵剧涓€涓潪NaN鍊?
        ...
        # 1. 绮剧‘鍖归厤锛圡49 + Item + Process + ParamName锛?
        mask = (
    ...
    def compute_crop_residues_n2o(self,
        ...
        """
        璁＄畻 Crop residues 鐩存帴 N2O 鎺掓斁
        
        鍗曚綅璇存槑锛堟潵鑷狦CE_parameters.xlsx锛夛細
        - Residue N content: kg N / tonne product
        - Emission factor: kg N2O / kg N (娉ㄦ剰锛氱洿鎺ユ槸N2O锛屼笉鏄疦2O-N)
        
        璁＄畻鍏紡锛?
        1. residue_n_kg = production_t 脳 Residue_N_content  [tonne 脳 kg/tonne = kg N]
        2. n2o_kg = residue_n_kg 脳 EF                       [kg N 脳 kg N2O/kg N = kg N2O]
        3. n2o_kt = n2o_kg / 1e6                            [kg 鈫?kt]
        """
    ...
    def compute_burning_ch4_n2o(self,
                               ...
                               scenario_params: Optional[Dict] = None) -> pd.DataFrame:
        """
        璁＄畻 Burning crop residues 鐨?CH4 鍜?N2O 鎺掓斁
        ...
        璁＄畻鍏紡锛?
        1. biomass_dm_kg = production_t 脳 dm_content   [tonne 脳 kg/tonne = kg DM]
        2. ch4_kg = biomass_dm_kg 脳 ef_ch4             [kg DM 脳 kg CH4/kg DM = kg CH4]
        3. n2o_kg = biomass_dm_kg 脳 ef_n2o             [kg DM 脳 kg N2O/kg DM = kg N2O]
        
        娉ㄦ剰锛氬綋鍓嶅弬鏁拌〃鍙兘涓嶅尯鍒?CH4/N2O 鐨?EF锛屾殏鏃朵娇鐢?IPCC 榛樿姣斾緥
        IPCC 鍏稿瀷鍊硷細EF_CH4 鈮?0.0027 kg/kg DM, EF_N2O 鈮?0.00007 kg/kg DM
        """
    ...
    def compute_rice_ch4(self,
                        ...
                        scenario_params: Optional[Dict] = None) -> pd.DataFrame:
        """
        璁＄畻 Rice cultivation 鐨?CH4 鎺掓斁
        
        鍗曚綅璇存槑锛堟潵鑷狦CE_parameters.xlsx锛夛細
        - Emission factor: kg CH4/ha
        
        璁＄畻鍏紡锛?
        1. ch4_kg = area_ha 脳 EF     [ha 脳 kg/ha = kg CH4]
        2. ch4_kt = ch4_kg / 1e6     [kg 鈫?kt]
        """
    ...
    def compute_synthetic_fert_n2o(self,
                                  ...
                                  scenario_params: Optional[Dict] = None) -> pd.DataFrame:
        """
        璁＄畻 Synthetic fertilizers 鐨?N2O 鎺掓斁锛堟湭鏉ュ勾浠斤級
        
        閫昏緫锛?
        1. 浣跨敤 synthetic_fertilizer_direct_N2O 杩囩鍙傛暟
        2. 瀵规瘡涓?M49-Item 璇诲彇鏂借偉鐜囧拰鎺掓斁鍥犲瓙
        3. 璁＄畻 N2O 鐩存帴鎺掓斁
        """
    ...
    def _get_historical_emissions(self, year: int) -> Dict[str, pd.DataFrame]:
        """
        浠庡巻鍙叉帓鏀炬枃浠朵腑璇诲彇鎸囧畾骞翠唤鐨勬帓鏀炬暟鎹?
        
        鏂囦欢鏍煎紡锛氬鏍煎紡锛屾瘡骞翠竴鍒?(Y2000, Y2001, ...)
        Element鍒楀寘鍚? 'Crop residues (Emissions N2O)', 'Burning crop residues (Emissions CH4)', etc.
        
        杩斿洖: {杩囩鍚? DataFrame}锛屾暟鎹凡鏍囧噯鍖栦负 (M49, Item, year, process, CH4_kt, N2O_kt, CO2_kt)
        """
        ...
        # 妫€鏌ユ槸鍚︽槸瀹芥牸寮忥紙鏈夊勾浠藉垪濡俌2000, Y2001绛夛級
        ...
        # 涓烘瘡涓繃绋嬪悎骞跺苟閫忚鎴?CH4_kt, N2O_kt, CO2_kt 鍒?
        for process_name in ['Crop residues', 'Burning crop residues', 'Rice cultivation', 'Synthetic fertilizers']:
            ...
            # Synthetic fertilizers鐗规畩澶勭悊锛氭寜19绉岻tem鍒嗛厤
    ...
    def _allocate_synthetic_fertilizers_by_items(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        """
        鎸?9绉岻tem鍒嗛厤Synthetic fertilizers鍘嗗彶鎺掓斁
        
        閫昏緫锛?
        1. 璇诲彇Fertilizer_efficiency.xlsx涓殑N_contentModi_Yxxxx鍒?
        2. 浣跨敤dict_v3鐨処tem_Fertilizer_Map鏄犲皠灏咺tem鍚嶇О鏍囧噯鍖栦负Item_Emis
        3. 瀵逛簬鏃犳硶鏄犲皠鐨処tem锛堝Others_crops锛夛紝灏嗗叾N鏂界敤閲忕瓑姣斾緥鍒嗛厤鍒板叾浠栧彲鏄犲皠鐨処tem涓?
        4. 瀵规瘡涓狹49_Country_Code锛岃绠楀悇Item鐨凬鏂界敤閲忓崰姣?
        5. 鐢ㄥ崰姣斿垎閰嶆€绘帓鏀惧埌鏍囧噯鍖栧悗鐨処tem
        
        Args:
            df: 鍘熷鎺掓斁鏁版嵁锛圛tem='Nutrient nitrogen N (total)'锛?
            year: 骞翠唤
        
        Returns:
            鍒嗛厤鍚庣殑鎺掓斁鏁版嵁锛堜娇鐢tem_Emis鏍囧噯鍚嶇О锛?
        """
        ...
        # 鍘熼€昏緫锛氬皢鏃犳硶鏄犲皠鐨処tem鐨凬鏂界敤閲忕瓑姣斾緥鍒嗛厤鍒板叾浠栧彲鏄犲皠鐨処tem
        # 鐜伴€昏緫锛氱洿鎺ヨ垗寮冩棤娉曟槧灏勭殑Item锛屽彧淇濈暀鍙槧灏勭殑閮ㄥ垎
        ...
        fert_eff_redistributed = fert_eff_redistributed[fert_eff_redistributed['share'] > 0]  # 绉婚櫎闆跺崰姣?
        
        # 鍚堝苟鎺掓斁鏁版嵁锛堥€氬父Item='Nutrient nitrogen N (total)'锛?
        # 鎻愬彇鎬绘帓鏀?
        total_emis = df[df['Item'].str.contains('Nutrient nitrogen N', na=False, case=False)].copy()
        
        if total_emis.empty:
            # 濡傛灉娌℃湁total锛岃繑鍥炲師鏁版嵁
            return df
    ...
    def _get_fertilizer_item_mapping(self) -> dict:
        """
        浠?dict_v3 鐨?Emis_item sheet 鑾峰彇 Item_Fertilizer_Map -> Item_Emis 鐨勬槧灏?
    ...
    def run_full_calculation(self,
        ...
        閫昏緫锛?
        1. 鍘嗗彶骞翠唤 (鈮?020): 鐩存帴浠?Emissions_crops_E_All_Data_NOFLAG.csv 璇诲彇
        2. 鏈潵骞翠唤 (>2020): 閫氳繃鍙傛暟璁＄畻寰楀埌
        
        Args:
            production_df: production 鏁版嵁 (M49_Country_Code, Item, year, production_t)
            harvest_area_df: harvest area 鏁版嵁 (M49_Country_Code, Item, year, harvested_area_ha)
            years: 璁＄畻骞翠唤鍒楄〃
            scenario_params: 鎯呮櫙鍙傛暟
...
def run_crop_emissions(production_df: pd.DataFrame,
                      ...
                      scenario_params: Optional[Dict] = None) -> Dict[str, pd.DataFrame]:
    """
    涓诲嚱鏁帮細杩愯 Crop 鎺掓斁璁＄畻
    
    Args:
        production_df: production 鏁版嵁妗?
        harvest_area_df: harvest area 鏁版嵁妗?
        years: 璁＄畻骞翠唤
        gle_params_path: 鍙傛暟鏂囦欢璺緞
        dict_v3_path: dict_v3 鏂囦欢璺緞
        hist_emissions_crop_path: 鍘嗗彶鎺掓斁 CSV 璺緞
        fertilizer_efficiency_path: Fertilizer_efficiency.xlsx璺緞锛堢敤浜嶴ynthetic fertilizers鍘嗗彶鍒嗛厤锛?
        scenario_params: 鎯呮櫙鍙傛暟
    
    Returns:
        {process_name: DataFrame} 瀛楀吀
    """
